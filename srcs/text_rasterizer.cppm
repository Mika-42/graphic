...
public:
  ...
  [[nodiscard]] bool getLineMetrics(const std::string &fontPath,
    ...
    outMetrics.ascender =
        static_cast<float>(cache->face->size->metrics.ascender >> 6);
    outMetrics.descender =
        static_cast<float>(cache->face->size->metrics.descender >> 6);
  ...
  [[nodiscard]] const TextGlyph *getOrCreateGlyph(const std::string &fontPath,
    ...
    if (FT_Load_Char(cache->face, codepoint, FT_LOAD_RENDER) != 0) {
      DEBUG_LOG("TextRasterizer: FT_Load_Char failed for codepoint " +
    ...
    newGlyph.payload.size = glm::vec2(static_cast<float>(bitmap.width),
                                      static_cast<float>(bitmap.rows));
    newGlyph.payload.bearing = glm::vec2(static_cast<float>(glyph->bitmap_left),
    ...
    if (bitmap.width > 0 && bitmap.rows > 0) {
      ...
      for (int y = 0; y < static_cast<int>(bitmap.rows); ++y) {
        for (int x = 0; x < static_cast<int>(bitmap.width); ++x) {
          const size_t srcIndex =
              static_cast<size_t>(y) * bitmap.pitch + static_cast<size_t>(x);
          const size_t dstIndex =
              (static_cast<size_t>(y) * bitmap.width + static_cast<size_t>(x)) *
              4u;
          const unsigned int coverage =
              static_cast<unsigned int>(bitmap.buffer[srcIndex]);
      ...
      if (!createGlyphTextureRGBA(rgba.data(), static_cast<int>(bitmap.width),
...
private:
  ...
  [[nodiscard]] FontCache *getOrCreateFontCache(const std::string &fontPath) {
    if (ftLibrary == nullptr || fontPath.empty()) {
      if (ftLibrary == nullptr) {
        DEBUG_LOG("TextRasterizer: FreeType is not initialized.");
	  }
	  if (fontPath.empty()) {
        DEBUG_LOG("TextRasterizer: font path is empty.");
	  }
      return nullptr;
    }
  ...
  [[nodiscard]] bool setPixelSize(FontCache &cache,
                                  unsigned int pixelSize) const {
    if (FT_Set_Pixel_Sizes(cache.face, 0, pixelSize) != 0) {
      DEBUG_LOG("TextRasterizer: FT_Set_Pixel_Sizes failed for size " +
  ...
  [[nodiscard]] bool createGlyphTextureRGBA(const unsigned char *rgbaPixels,
                                            int width, int height,
                                            GLuint &outTextureId,
                                            uint64_t &outTextureHandle) const {
