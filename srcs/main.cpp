...
public:
  ...
  void render(const glm::vec2 &size, const MouseEventView & /*mouse*/,
    ...
    if (keyboard.isPressed(Key::Down)) {
      player.geometry.y += speed;
      player.geometry.y =
          glm::min(player.geometry.y, size.y - player.geometry.w);
    }
    ...
    if (keyboard.isPressed(Key::Right)) {
      player.geometry.x += speed;
      player.geometry.x =
          glm::min(player.geometry.x, size.x - player.geometry.z);
    }

    player.shadowSoftness =
        50.0f + 30.0f * glm::abs(glm::cos(glm::pi<float>() * 0.125f * time.now /
                                          1000.0f));
    ...
    hitbox.geometry = {
        ...
        2.0f * player.shadowSoftness, 2.0f * player.shadowSoftness};
    ...
    for (size_t i = 0; i < DOTS_COUNT; ++i) {
      ...
      dots[i].shadowSoftness =
          50.0f + 30.0f * glm::abs(glm::cos(glm::pi<float>() * 0.25f *
                                                time.now / 1000.0f +
                                            50.0f * float(i)));
  ...
  void moveDot(gl::Rectangle &dot, const glm::vec2 &p) {
    if (dot.geometry.x > p.x)
      dot.geometry.x -= speed;
    if (dot.geometry.y > p.y)
      dot.geometry.y -= speed;
    if (dot.geometry.x < p.x)
      dot.geometry.x += speed;
    if (dot.geometry.y < p.y)
      dot.geometry.y += speed;
...
private:
  ...
  const char *IMG_PATH = "/home/mika/project/cpp/graphic/example/"
                         "Lofi-Forest-Wallpaper-4K-Desktop-2.jpg";
